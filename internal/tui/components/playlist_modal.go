...
// HandleKey processes a key press (string), returns (handled, shouldClose, shouldCreate)
// Deprecated: Use HandleKeyMsg for proper textinput handling
func (m *PlaylistModal) HandleKey(key string) (handled bool, shouldClose bool, shouldCreate bool) {
	if !m.visible {
		return false, false, false
	}

	// Handle create mode - we can't properly route to textinput without the full KeyMsg
	if m.createMode {
		switch key {
		case "esc":
			m.createMode = false
		...
		// For other keys, we need the full KeyMsg
		return true, false, false
	}
